...
using Cody_v2.Services.RequestDTOs.User;
...
namespace Cody_v2.Web.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public async Task<IActionResult> AddRoleAsync(string id, [Bind("RoleNames")] AddUserRoleModel model)
        {
            ...
            {
                ModelState.AddModelError(ErrorKey, resultDelete.ToString());
                return View(model);
            }
            ...
            {
                ModelState.AddModelError(ErrorKey, resultAdd.ToString());
                return View(model);
            }
        ...
        public async Task<IActionResult> EditClaim(int claimid, AddUserClaimModel model)
        {
            ...
            if (_context.UserClaims.Any(c => c.UserId == user.Id 
                ...
                && c.Id != userclaim.Id))
                {
                    ModelState.AddModelError(ErrorKey, "Claim này đã có");
                    return View("AddClaim", model);
                }
